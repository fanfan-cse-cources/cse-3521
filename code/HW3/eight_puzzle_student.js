...
function is_goal_state(state) {
  ...
  for(r = 0; r < 3; r++){
    for(c = 0; c < 3; c++){
      ...
      if(state.grid[r][c] != goal[index]){
        return false;
      }
      index++;
    }
  }
  return true;/***Your code to check for goal state here!***/;
}
...
function find_successors(state) {
  ...
  if(row != 0){
    ...
    //newState.estimated_total_cost = calculate_heuristic(newState);
