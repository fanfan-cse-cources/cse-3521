  ...
  for(let i=0;i<N;++i) {
    ...
    ***********************/
    ...
    for(let j = 0; j <= order; j++){
      //not enough data points for polynomial(identical columns will cause bug)
      A[i][j] = j == 0 ? 1 : Math.pow(x[i], j);
    }
  }
...
function calc_jacobian(data,p) {
  ...
  for(let i=0;i<N;++i) {
    ...
    */
    ...
    J[i][2] = p[3] * Math.log(x[i]) * Math.pow(x[i], p[2]);
    J[i][1] = x[i];
    J[i][0] = 1;
  }
...
function calc_nonlinLSQ_gradientdescent(data,initial_p,max_iterations,learning_rate) {
  ...
  for(let iter=0;iter<=max_iterations;++iter) {
    ...
    */
    let J = calc_jacobian(data,p);
    let grad = numeric.dot(numeric.transpose(J), dy);
    grad = grad.map(x=>x*2*learning_rate);
