...
function helper_check_student_source() {
  ...
  obj.lastChild.href = "https://www.google.com/search?q=Javascript+console+" + platform.os.toString()
    .replace(/\s+/g, '+') + "+" + platform.name.toString()
    .replace(/\s+/g, '+');
  ...
  if (game_board_is_terminal) {
    ...
    requestAnimationFrame(function () {
      requestAnimationFrame(function () { //Do in (2nd) anim callback to let screen update before hogging cpu
        ...
        let results;
        try {
          if (document.getElementById("input_cpumode_mm")
            .checked)
